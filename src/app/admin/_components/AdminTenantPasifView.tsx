...
export default function AdminTenantPasifView() {
  const handleSignOut = async () => {
    const supabase = createClient();
    const { error } = await supabase.auth.signOut();
    if (error) {
      console.error("[AdminTenantPasifView] signOut hatası:", error);
    }
        ...
        <div className="rounded-xl bg-white p-8 shadow-sm border border-border">
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold text-text-dark tracking-tight">
              Yönetim Paneli Kapalı
            </h1>
            <p className="text-sm text-text-muted mt-3 leading-relaxed">
              Bu kuruluşun yönetim paneli şu anda kapalı. Tenant yöneticiniz
              ile iletişime geçin.
            </p>
          </div>
